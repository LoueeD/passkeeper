...
Package releases run from the manual `.github/workflows/release.yml` GitHub Actions workflow. It requires npm trusted publishing via GitHub OIDC, so do not add npm access tokens to repository secrets or instruct contributors to publish from local credentials. The workflow runs `pnpm run release`; it can only publish from `main` when its `publish` confirmation input is supplied. Successful releases push the package tags created by Changesets. See `docs/releasing.md` for the maintainer runbook and recovery path.
